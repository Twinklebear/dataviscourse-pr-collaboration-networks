...
MainView.prototype.loadAuthorData = function(data_path){
	...
	d3.loadData()
		...
		.onload(function(data){
			...
			self.updateClustered();
		});
}

MainView.prototype.loadData = function(data_path, journal){
	var self = this;
	if(!this.authors_data)	this.loadAuthorData(data_path);
	d3.loadData()
...
MainView.prototype.displayAuthorInfo = function(d){
	var newline = '</br>';
	var author = authors_data[d]; 
	console.log(authors_data[d])
				...
				for(var j = 0; j<article.authors.length; j++){
					if(article.authors[j] != d)
						if(j<article.authors.length-1)
							msg += authors_data[article.authors[j]].name + ", ";
						else
							msg += authors_data[article.authors[j]].name;
				}
				msg+= newline;
...
MainView.prototype.updateClustered = function(){
	var self = this;
...
// --------------------------------------------------------
	var vis = d3.select("#bundle_nodes2").append("svg")
				.attr("width", width)
				.attr("height", height);
	...
	// var clusters = cluster_data;
	console.log(Object.keys(cluster_data.clusters).length)
	// for(var i = 0; i<clusters.length; i++){
		var i = 1;
		for(var j = 0; j<Object.keys(cluster_data.clusters).length; j++){
			var numAuthors = cluster_data.clusters["Cluster"+j].authors.length;
			for(var k = 0; k<numAuthors; k++){
				nodes.push({"name":authors_data[cluster_data.clusters["Cluster"+j].authors[k]].name, "group":i, "cluster":j, "node_id":cluster_data.clusters["Cluster"+j].authors[k]});
	...
	function init() {
		...
		var circle = node.enter().append("circle")
			...
			.on("click", function(d){
				if(!d.nodes)
					self.displayAuthorInfo(d.node_id);
			})
			.on("mousewheel.zoom", function(d) { d3.event.stopPropagation();
		    	var dcx = (width/2-d.x*zoom.scale());
		    	var dcy = (height/2-d.y*zoom.scale());
